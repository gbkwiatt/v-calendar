...
class theme_Theme {
  ...
  normalizeNonHighlight(type, config, styleFn) {
    ...
    toPairs_default()(attr).forEach(([_, targetConfig]) => {
...
function resolveConfig(config, locales) {
  ...
  config = isObject(config) ? defaultsDeep_default()(config, defLocale) : defLocale; // Set default mask if not provided with custom option
...
class locale_Locale {
  ...
  normalizeDates(dates, opts) {
    ...
    return (isArrayLikeObject_default()(dates) ? dates : [dates]).map(d => d && (d instanceof dateInfo_DateInfo ? d : new dateInfo_DateInfo(d, opts))).filter(d => d);
  }
